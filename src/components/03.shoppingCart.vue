                ...
                <div class="bg-wrap ">
                    ...
                    <div class="cart-foot clearfix ">
                        <div class="right-box ">
                            ...
                            <button class="button " @click="checkAndSubmit ">立即结算</button>
  ...
  created() {
    ...
    this.$axios.get(`site/comment/getshopcargoods/${ids}`).then(response => {
      ...
      response.data.message.forEach((v, i) => {
        // 获取 Vuex中的 id对应的值
        v.buycount = cartDate[v.id];//获取到有v.id这个属性名的值,就可以获得相对应数量
        //默认不被选中
        v.selected = false;
      });
  ...
  methods: {
    ...
    checkAndSubmit() {
      ...
      if (this.totalPrice == 0) {
        this.$message.error("未选择要买的东西!");
        return;
      }

      //到详情页面要带入东西的ID,把选中的id都传过去，用逗号隔开
      let ids = "";
      this.message.forEach(v => {
        if (v.selected) {
          ids += v.id;
      ...
      //把最后面的逗号删除
      ids = ids.slice(0, -1);
      //main.js会判断登录状态，如果有则执行以下代码，到订单页面
      this.$router.push("oder/" + ids);
